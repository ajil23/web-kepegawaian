    ...
    <div class="min-h-screen flex items-center justify-center px-4 py-10">
        <div class="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-100 p-8">
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input name="name" value="{{ old('name') }}" required
                        class="w-full mt-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-800 outline-none text-sm">
                    <x-input-error :messages="$errors->get('name')" class="mt-1" />
                </div>

                <div>
                    <label class="text-sm font-semibold text-slate-700">NIP</label>
                    <input name="nip" value="{{ old('nip') }}" required pattern="\d*" oninput="this.value = this.value.replace(/\D/g,'')" class="w-full mt-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-800 outline-none text-sm">
            ...
            <div class="text-center mt-6 text-sm text-slate-500">
                Sudah punya akun?
                <a href="{{ route('login') }}" class="text-sm font-medium text-green-800 hover:underline">
                    Login
                </a>
            </div>
        </div>
